    ...
    fields: [
      {
        ...
        placeholder: 'pat-na1-…',
        secret: true,
        help: 'Needs crm.objects.contacts read + write scopes (add deals read + write for auto-deals).',
      },
      {
        key: 'create_deals',
        label: 'Also create a deal when a lead is interested or books a meeting',
        placeholder: '',
        secret: false,
        optional: true,
        toggle: true,
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        key: 'database_id',
        label: 'Database',
        placeholder: 'Pick a database (or paste its 32-character ID)',
        secret: false,
        help: 'Remember to share the database with your integration via Connections.',
        picker: 'notion_database',
      },
    ],
    supportedEvents: CRM_EVENTS,
    defaultEvents: CRM_EVENTS,
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        key: 'base_id',
        label: 'Base',
        placeholder: 'Pick a base (or paste its app… ID)',
        secret: false,
        picker: 'airtable_base',
      },
      {
        key: 'table_name',
        label: 'Table',
        placeholder: 'Pick a table (or type its name)',
        secret: false,
        picker: 'airtable_table',
      },
    ],
    supportedEvents: CRM_EVENTS,
    defaultEvents: CRM_EVENTS,
