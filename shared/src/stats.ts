/* ═══════════════════════════════════════════════════════════════════════
   The small amount of statistics this product is allowed to do.

   Moved here from server/utils so the client can refuse to draw a
   conclusion for exactly the same reasons the server refuses to compute
   one. It was server-only, which meant every honesty rule built on it -
   the A/B winner, the bounce guard, the step verdict - stopped at the
   API boundary, and anything rendered in the browser was free to divide
   two numbers and call it a rate.

   server/utils/stats.ts re-exports these rather than keeping a copy. Two
   implementations of a confidence bound that agree until one is tuned is
   the same trap as two copies of imapHostFor.
   ═══════════════════════════════════════════════════════════════════════ */
