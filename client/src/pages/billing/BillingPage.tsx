...
import { CreditCard, Mail, Inbox, Check, X, Sparkles, Zap, Building2 } from 'lucide-react';
...
export function BillingPage() {
  ...
  const onUpgrade = (planId: PlanId) => {
    if (planId === 'scale') {
      window.location.href = 'mailto:info@affivault.com?subject=Sincerely%20Scale%20plan';
      return;
    }
    // Stripe Checkout is wired up in a follow-up. Until then, make the intent clear.
    toast('Checkout is being set up — hang tight, upgrades go live shortly.', { icon: '🛠️' });
  };
  ...
  return (
    <div>
      ...
      {/* Plans */}
      <h3 className="text-[13px] font-semibold text-[var(--text-primary)] mb-3">Plans</h3>
        ...
        {DISPLAY_PLANS.map((id) => {
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                <div className="mt-1 flex items-baseline gap-1">
                  {plan.priceMonthly === null ? (
                    ...
                    <>
                      <span className="text-[22px] font-bold text-[var(--text-primary)]">${plan.priceMonthly}</span>
                      <span className="text-[12px] text-[var(--text-tertiary)]">/mo</span>
                    </>
                  )}
                </div>
                {plan.priceAnnual !== null && plan.priceAnnual !== plan.priceMonthly && (
                  <p className="text-[11px] text-[var(--text-tertiary)] mt-0.5">${plan.priceAnnual}/mo billed annually</p>
                )}
              </div>
              ...
              >
                {isCurrent ? 'Current plan' : id === 'scale' ? 'Contact sales' : 'Upgrade'}
              </button>
            </div>
          );
        })}
