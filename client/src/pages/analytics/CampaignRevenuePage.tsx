          ...
          <div key={s.label} className="flex items-center gap-3">
            ...
            <div className="relative h-7 flex-1 overflow-hidden rounded-md bg-[var(--bg-elevated)]">
              ...
              {!first && s.ofPrevious !== null && (
                <span className={cn(
                  'absolute inset-y-0 flex items-center text-micro font-semibold tabular-nums',
            ...
            <span className="w-16 flex-shrink-0 text-right text-strong font-semibold tabular-nums text-[var(--text-primary)]">
...
function Stat({ label, value, sub, tone }: { label: string; value: string; sub?: string; tone?: 'money' }) {
  return (
    <div className="card px-4 py-3.5">
      ...
      <p className={cn(
        'mt-1.5 text-title font-semibold leading-none tabular-nums',
...
export function CampaignRevenuePage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <Link to="/analytics/revenue" className="inline-flex items-center gap-1 text-body font-medium text-[var(--text-tertiary)] hover:text-[var(--text-primary)]">
          <ArrowLeft className="h-3 w-3" /> Revenue
        </Link>
        <h1 className="mt-1.5 text-display font-semibold tracking-[-0.02em] text-[var(--text-primary)]">{campaign.name}</h1>
      ...
      <div className="panel p-4">
        <h2 className="mb-3 text-caption font-bold text-[var(--text-tertiary)]">Sent to won</h2>
        <Funnel steps={funnel} />
      </div>

      <div className="panel overflow-hidden">
        <div className="flex items-center justify-between gap-2 border-b border-[var(--border-subtle)] px-4 py-2.5">
          <h2 className="text-caption font-bold text-[var(--text-tertiary)]">Which step earned it</h2>
          {carried && (
            ...
            <tbody>
              {steps.map((st: StepRevenueRow) => (
                <tr key={st.id} className={cn(
                  ...
                  <td className="py-2.5 pl-4 pr-3 max-w-[300px]">
                    <span className="text-body font-semibold text-[var(--text-primary)]">Step {st.step_order}</span>
                    {st.subject && (
                      <p className="truncate text-caption text-[var(--text-tertiary)]">{st.subject}</p>
                    )}
                  </td>
                  <td className="py-2.5 px-3 text-right text-body tabular-nums text-[var(--text-tertiary)]">{st.sent.toLocaleString()}</td>
                  <td className="py-2.5 px-3 text-right text-body tabular-nums text-[var(--text-secondary)]">{st.replied.toLocaleString()}</td>
                  <td className="py-2.5 px-3 text-right text-body tabular-nums text-[var(--text-secondary)]">{st.deals.toLocaleString()}</td>
              ...
              {unrecorded && (
                <tr className="border-b border-[var(--border-subtle)] last:border-0">
                  ...
                  <td />
                  <td className="py-2.5 pl-3 pr-4 text-right text-strong font-semibold tabular-nums text-[var(--text-tertiary)]">
      ...
      <div className="panel overflow-hidden">
        <div className="border-b border-[var(--border-subtle)] px-4 py-2.5">
          <h2 className="text-caption font-bold text-[var(--text-tertiary)]">
            ...
            {deals.map((d: AttributedDealRow) => (
              ...
              >
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-body font-semibold text-[var(--text-primary)]">{d.title}</span>
                  {(d.contact_name || d.contact_email) && (
                    <span className="block truncate text-caption text-[var(--text-tertiary)]">{d.contact_name || d.contact_email}</span>
                  )}
                </span>
                {d.attribution && (
                  <span className={cn('inline-flex flex-shrink-0 items-center px-1.5 h-[19px] rounded-md text-micro font-semibold', ATTR_TONE[d.attribution])}>
                    {ATTRIBUTION_LABEL[d.attribution]}
                  </span>
                )}
                <span className={cn(
                  'w-24 flex-shrink-0 text-right text-body font-semibold tabular-nums',
